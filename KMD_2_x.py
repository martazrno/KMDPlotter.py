...
#find distance of extra scatter
def find_dist_extra(df_all):
    max_KM = df_all['KM_y'].max()
    min_KM = df_all['KM_y'].min()
    min_KMD = df_all.loc[df_all['KM_y'] == min_KM, 'KMD_y'].values[0]
    max_KMD = df_all.loc[df_all['KM_y'] == max_KM, 'KMD_y'].values[0]

    distances = abs((max_KM - min_KM) * (min_KMD - df_all['KMD_y_extra']) -
                        (min_KM - df_all['KM_y_extra']) * (max_KMD - min_KMD)) / math.sqrt((max_KM - min_KM) ** 2 + (max_KMD - min_KMD) ** 2)
    return distances
...
def append_to_csv(filename, points_b, distances_b, points_y, distances_y, points_b2, distances_b2, points_y2, distances_y2, points_y_extra, distances_y_extra):
    points_b = list(points_b)
    points_y = list(points_y)
    points_b2 = list(points_b2)
    points_y2 = list(points_y2)
    points_y_extra = list(points_y_extra)

    max_length = max(len(points_b), len(distances_b), len(points_y), len(distances_y), len(points_b2), len(distances_b2), len(points_y2), len(distances_y2), len(points_y_extra), len(distances_y_extra))
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        writer.writerow(['Limit 1 b KM', 'Limit 1 b KMD', 'Limit 1 b distances', '', 'Limit 1 y KM', 'Limit 1 y KMD',
                         ...
                         'Limit 2 y KM', 'Limit 2 y KMD', 'Limit 2 y distances', '', 'Extra y KM', 'Extra y KMD',
                         'Extra y distances'])

        for i in range(max_length):
            row = []

            for points, distances in [(points_b, distances_b), (points_y, distances_y), (points_b2, distances_b2), (points_y2, distances_y2), (points_y_extra, distances_y_extra)]:
                point_x = "{:.4f}".format(points[i][0]) if i < len(points) and points[i][0] is not None and not np.ma.is_masked(points[i][0]) and not math.isnan(points[i][0]) else ''
                point_y = "{:.4f}".format(points[i][1] / 1000) if i < len(points) and points[i][1] is not None and not np.ma.is_masked(points[i][1]) and not math.isnan(points[i][1]) else ''
                distance = "{:.4f}".format(distances[i] / 1000) if i < len(distances) and distances[i] is not None and not math.isnan(distances[i]) else ''
...
def plot_graph(file_all):
    ...
    df_all.columns = ['Mass', 'Intensity', 'Mass_b', 'Mass_y', 'Extra_y']
    ...
    df_all['KM_y_extra'], df_all['KMD_y_extra'] = convert_to_km_kmd(df_all, 'Extra_y')
    fig, ax = plt.subplots(figsize=(11, 7))
    ax.set_position([0.1, 0.1, 0.69, 0.82])
    ...
    scatters_extra_y = ax.scatter(df_all['KM_y_extra'], df_all['KMD_y_extra'], color='purple', s=13, visible=False)
    ...
    # save scatter button
    ax_button = plt.axes([0.81, 0.15, 0.13, 0.05])
    btn = Button(ax_button, 'Save New Scatter')

    # checkboxes
    check_ax = plt.axes([0.8, 0.685, 0.15, 0.2], frameon=True, facecolor='#EDEDED')
    check = CheckButtons(check_ax,['B and y lines','Extra y scatter' ,'Max. distance', 'Limit 1 area','Limit 1 scatter', 'Limit 2 area', 'Limit 2 scatter'], [False, False, False, False, False, False, False])
    ...
    def update_lines(label):
        ...
        elif label == 'Extra y scatter':
            scatters_extra_y.set_visible(not scatters_extra_y.get_visible())
    ...
    extra_points = scatters_extra_y.get_offsets()
    ...
    def save_scatter(event):
        ...
        extra_distances = find_dist_extra(df_all)
        filename = easygui.filesavebox(msg='Choose name for scatter file:', default='*.csv')
        if filename:
            append_to_csv(filename, points_within_b, just_distances_b, points_within_y, just_distances_y, points_within_b2, just_distances_b2, points_within_y2, just_distances_y2, extra_points, extra_distances)
    ...
    text_box = TextBox(plt.axes([0.879, 0.5, 0.1, 0.04]), 'Enter mass:')
    text_box.on_submit(submit)

    delete_box = TextBox(plt.axes([0.881, 0.43, 0.1, 0.04]), 'Delete mass:')
